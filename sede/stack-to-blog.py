...
EXTRA_SEARCH_FILES = [
    '../about.md', '../answers.md', '../hrb.md', '../index.md', '../programs.md',
    '../programs/homa.md', '../programs/hyperlink.md', '../programs/iothings.md',
    '../programs/mserve.md', '../programs/mt.md', '../programs/stack.md',
...
def check_full_links(ln):
    ...
    while True:
        ...
        if post_url is not None:
            total_full_links += 1
            search_str = "[" + name + "]"
            replace_str = "[" + str(ss_title) + "]"  # str() for pycharm err
            ln = ln.replace(search_str, replace_str)
            search_str = "(" + found + ")"
            replace_str = "(" + str(ss_post_url) + ")"  # str() for pycharm
            ln = ln.replace(search_str, replace_str)
        else:
...
https://askubuntu.com/a/1039377/307523
    ...
    if get_ss_url(search_url, search_type="Answer"):
        ...
        #print('Found Question URL:', search_url)
        # Now get our answer matching question's title
        if get_ss_title(str(ss_title), search_type="Answer"):
...
def front_matter(r):
    ...
    if FRONT_GIT_URL is not None:
        # str() usage below for pycharm warning of type None
        md += FRONT_GIT_URL + str(code_url) + base_filename + ".md" + '\n'
...
def extra_file_as_post_init(extra):
    ...
    final_url = str(html_url) + "/" + basename + ".html"
